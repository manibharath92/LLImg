...
class PatchEmbed(nn.Module):
    def __init__(self, img_size=64, patch_size =2, dim=96, norm_layer=None):
        super().__init__()
...
class TransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type, window_size=7, shift_size=0, qkv_bias=True, qk_scale=None):
        super(TransformerBlock, self).__init__()

        self.patch_embed = PatchEmbed(dim=dim*2, norm_layer=None)  #nn.LayerNorm
        self.patch_unembed = PatchUnEmbed(dim=dim*2)
        ...
        self.attn_window = WindowAttension(dim=dim*2, window_size=to_2tuple(window_size), num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale)
    ...
    def forward(self, x, y, t):
        ...
        x_init = torch.cat([x, y], 1)
        x = modulate(self.norm1(x_init), shift_msa, scale_msa)
        ...
        x_windows = x_windows.view(-1, self.window_size * self.window_size, 2*C)  # nW*B, window_size*window_size, C
        ...
        if self.input_resolution == x_size:
            ...
            attn_windows = self.attn_window(x_windows, mask=self.calculate_mask(x_size).to(x.device))
        ...
        x = x_init + gate_msa.unsqueeze(-1).unsqueeze(-1)  * x
...
class DiT_incontext_revise(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            TransformerBlock(dim=embed_dim, num_heads=num_heads, ffn_expansion_factor=mlp_ratio, bias=bias, LayerNorm_type=LayerNorm_type,
                             window_size=window_size, shift_size=0 if (i % 2 ==0) else window_size //2)  for i in range(depth)
        ])
